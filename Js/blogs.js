...
function setupBlogFilters() {
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Mostrar u ocultar las secciones según el filtro
            if (filter === 'all') {
...
// Efecto glitch aleatorio en textos
setInterval(() => {
    const blogTitles = document.querySelectorAll('.blog-title');
    const randomIndex = Math.floor(Math.random() * blogTitles.length);
    
    blogTitles[randomIndex].classList.add('glitched');
    
    setTimeout(() => {
        blogTitles[randomIndex].classList.remove('glitched');
    }, 1000);
}, 5000);
